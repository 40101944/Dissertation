...
public final class zzart
{
  ...
  {
    if (paramInt >= 0) {}
    for (paramInt = zzahe(paramInt);; paramInt = 10) {
      return paramInt;
    }
  }
  ...
  {
    int m = paramCharSequence.length();
    for (int i = 0; (i < m) && (paramCharSequence.charAt(i) < ''); i++) {}
    for (;;)
    {
      int k = i;
      int j;
    ...
    throws IOException
  {
    ...
    try
    {
      k = zzahe(paramString.length());
      if (k != zzahe(paramString.length() * 3)) {
        break label155;
      }
      j = this.btF.position();
      if (this.btF.remaining() < k)
      {
        paramString = new com/google/android/gms/internal/zzart$zza;
        paramString.<init>(k + j, this.btF.limit());
        throw paramString;
      }
    }
    ...
    this.btF.position(j + k);
    zza(paramString, this.btF);
    int i = this.btF.position();
    this.btF.position(j);
    zzahd(i - j - k);
    this.btF.position(i);
    for (;;)
...
/* Location:              C:\ApkExtract\classes-dex2jar.jar!\com\google\android\gms\internal\zzart.class
