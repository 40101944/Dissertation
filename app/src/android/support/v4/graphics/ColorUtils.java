...
public final class ColorUtils
{
  ...
  public static void RGBToHSL(@IntRange(from=0L, to=255L) int paramInt1, @IntRange(from=0L, to=255L) int paramInt2, @IntRange(from=0L, to=255L) int paramInt3, @NonNull float[] paramArrayOfFloat)
  {
    ...
    float f7 = Math.max(f3, Math.max(f5, f1));
    float f6 = Math.min(f3, Math.min(f5, f1));
    float f2 = f7 - f6;
    float f4 = (f7 + f6) / 2.0F;
    if (f7 == f6)
    {
  ...
  public static int blendARGB(@ColorInt int paramInt1, @ColorInt int paramInt2, @FloatRange(from=0.0D, to=1.0D) float paramFloat)
  {
    ...
    float f9 = Color.alpha(paramInt2);
    float f2 = Color.red(paramInt1);
    float f3 = Color.red(paramInt2);
    float f8 = Color.green(paramInt1);
    float f4 = Color.green(paramInt2);
    float f5 = Color.blue(paramInt1);
    float f7 = Color.blue(paramInt2);
    return Color.argb((int)(f6 * f1 + f9 * paramFloat), (int)(f2 * f1 + f3 * paramFloat), (int)(f8 * f1 + f4 * paramFloat), (int)(f5 * f1 + f7 * paramFloat));
  }
  ...
  public static int compositeColors(@ColorInt int paramInt1, @ColorInt int paramInt2)
  {
    ...
    int i = compositeAlpha(k, j);
    return Color.argb(i, compositeComponent(Color.red(paramInt1), k, Color.red(paramInt2), j, i), compositeComponent(Color.green(paramInt1), k, Color.green(paramInt2), j, i), compositeComponent(Color.blue(paramInt1), k, Color.blue(paramInt2), j, i));
  }
  ...
  private static double pivotXyzComponent(double paramDouble)
  {
    ...
    for (paramDouble = Math.pow(paramDouble, 0.3333333333333333D);; paramDouble = (903.3D * paramDouble + 16.0D) / 116.0D) {
      return paramDouble;
    }
  }
...
/* Location:              C:\ApkExtract\classes-dex2jar.jar!\android\support\v4\graphics\ColorUtils.class
